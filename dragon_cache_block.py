...
STATE_MACHINE = {
    INVALID: {
        ...
        PRRD_TO_SC: (SHARED_CLEAN, False, 100),
        PRRD_TO_E: (EXCLUSIVE, False, 100),
        PRWR_I_TO_M: (MODIFIED, False, 100),
        PRWR_I_TO_SM: (SHARED_MODIFIED, False, 100),
    },
    EXCLUSIVE: {
...
class CacheBlock(BaseCacheBlock):
    ...
    def prrd(self, origin=None):
        ...
        elif self.state == INVALID and not self.cache.is_any_shared(self.id):
    ...
    def prwr(self, origin=None):
        if self.state == SHARED_MODIFIED and self.cache.is_any_shared(self.id):
            self.state, bus_txn, cycles = self.step(PRWR_SM_TO_SM)
        elif self.state == SHARED_MODIFIED and not self.cache.is_any_shared(self.id):
            self.state, bus_txn, cycles = self.step(PRWR_SM_TO_M)
        elif self.state == INVALID and self.cache.is_any_shared(self.id):
            self.state, bus_txn, cycles = self.step(PRWR_I_TO_SM)
        elif self.state == INVALID and not self.cache.is_any_shared(self.id):
            self.state, bus_txn, cycles = self.step(PRWR_I_TO_M)
        elif self.state == MODIFIED and not self.cache.is_any_shared(self.id):
